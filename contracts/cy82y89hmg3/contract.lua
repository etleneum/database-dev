...
function buy_ticket ()
    ...
    do 
        local ticket_number = _table_length(auction_info.tickets)
        auction_info.tickets[ticket_number] = account.id
        auction_info.lottery_fund = auction_info.lottery_fund + ticket_price
    end

    if auction_info.tickets_to_start <= _table_length(auction_info.tickets) and not auction_info.started then
...
function finish_loto_auction ()
    ...
    local winner = auction_info.tickets[math.random(_table_length(auction_info.tickets))]
    util.print(_table_length(auction_info.tickets))
    util.print(math.random(_table_length(auction_info.tickets)))
    util.print(winner)
