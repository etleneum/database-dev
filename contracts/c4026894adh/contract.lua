...
function buy_ticket ()
    ...
    local tlength = table_length(auction_info.tickets)
    util.print(tlength)

    if auction_info.tickets_to_start <= table_length(auction_info.tickets) and not auction_info.started then
...
function finish_loto_auction ()
    ...
    local winner = auction_info.tickets[math.random(table_length(auction_info.tickets))]
