...
function deposit ()
  ...
  contract.state.pots[account.id] = contract.state.pots[account.id] or 0
  contract.state.pots[account.id] = contract.state.pots[account.id] + call.msatoshi
end
...
function loan_approve ()
  ...
  if not contract.state.pots[account.id] then
...
function loan_withdraw ()
  ...
  for _, _ in pairs(contract.state.pots) do
  ...
  if total_approvals == total_members then
    contract.send(loan.amount, account.id)
